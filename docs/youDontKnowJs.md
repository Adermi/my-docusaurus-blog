...
- 作用域中的变量的查找方式是向上级作用域查找，不能向同级作用域查找
  ...
  PS：
  <img src={require('/assets/youdontknowJS.assets/image-20210321213542416.png').default} alt="Example banner"/>
...
![Example banner](/assets/youdontknowJS.assets/image-20210319161900803.png)
...
![Example banner](/assets/youdontknowJS.assets/image-20210319164251433.png)
...
![Example banner](/assets/youdontknowJS.assets/image-20210321213514902.png)
...
![Example banner](/assets/youdontknowJS.assets/image-20210319171934505.png)
...
1. 隐藏函数内部实现，只暴露一个接口。类似黑盒，把处理数据的代码封装到一个黑盒中，只暴露入口（参数）和出口（return）
   ...
   ![Example banner](/assets/youdontknowJS.assets/image-20210321211835081.png)

2. 避免变量冲突。一个函数作用域中可各种变量和函数，并且把这些变量和全局作用域隔绝，不会污染全局作用域

   ![Example banner](/assets/youdontknowJS.assets/image-20210321211848684.png)
   
3. 闭包。用过的都说好，函数执行后对外 **return** 一个对象或者函数，用来突破函数作用域不能被外部作用域访问的限制

   ![Example banner](/assets/youdontknowJS.assets/image-20210321211908384.png)
   
4. 保护函数作用域中的变量（外部无法访问内部）

   ![Example banner](/assets/youdontknowJS.assets/image-20210321211918023.png)
...
![Example banner](/assets/youdontknowJS.assets/image-20210321205902958.png)
...
![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321211750152.png)
...
- 首先看 **var** 的变量提升【只提升申明，不赋值】
  ...
  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321212910717.png)
  
  其中的原因就是，编译器首先会把所有的 var 申明提升到作用域最前面，再执行代码
  
  真实运行代码的状况其实是这样的：

  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321212922149.png)

- 再来看 **function** 关键字的提升【又提升申明，又赋值】

  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321212932672.png)
  
  用function声明的函数，也会像var一样提升到最前面，但是这个关键字是 `提升+申明`
  
  代码真实运行情况：
  
  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321212942477.png)

- 两者提升的优先级

  > function 的提升优先级 > var关键字优先级

  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213003059.png)

  上面代码的真实运行情况

  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213014416.png)

- 匿名函数没有提升

  当我们使用function关键字作为匿名函数时，没有变量提升

  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213025727.png)
  
  但是！有名函数就会有提升
  
  ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213035664.png)
...
![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213110006.png)

:star: 请记住一点：在块级作用域中的匿名**有名函数没有提升**

![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213122272.png)
...
1. 来看一个闭包突破作用域限制的例子：
   ...
   ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213139799.png)
...
2. 闭包产生的原理
   ...
   ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321180039296.png)

3. 闭包的运用：

   - 各种回调函数

     ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213207381.png)
     
   - 自执行函数
   
     ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213217885.png)
     
   - 柯理化函数
   
     ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213228839.png)
     
   - 模块模式
   
     ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210321213240678.png)
...
6. 箭头函数的 `this` 会在箭头函数执行时捕获当前所在的执行栈中的 `this`【不能强制改变】
   ...
   ![image-20210321211750152](/assets/youdontknowJS.assets/image-20210322005239380.png)
...
![image-20210321211750152](/assets/youdontknowJS.assets/image-20210323231508326.png)
...
3. 多态：子类可以重写父类的方法；（比如跑车子类，他可以写一个他自己的发动引擎的模式，比如三喷起步十喷起步等等）
   ...
   ![Example banner](/assets/youdontknowJS.assets/image-20210326234402297.png)
...
![image-20210330154809674](/assets/youdontknowJS.assets/image-20210330154809674.png)
